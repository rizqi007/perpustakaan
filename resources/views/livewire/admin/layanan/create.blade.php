        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden p-6">
            <form wire:submit.prevent="save">
                ...
                <div class="mb-4">
                    ...
                    <input type="url" wire:model="url" id="url" placeholder="https://..." class="w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:text-white">
                    @error('url') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>

                <div class="mb-6">
                    <label for="image" class="block text-gray-700 dark:text-gray-300 font-bold mb-2">Icon / Gambar</label>
                    ...
                    @if ($image)
                        <div class="mt-2">
                            <img src="{{ $image->temporaryUrl() }}" class="h-24 w-auto object-contain rounded border p-2">
                        </div>
                    @endif
